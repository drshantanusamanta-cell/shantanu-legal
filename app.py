...
# ---------------------------------------------------------------------------
# Import bootstrap + preflight
#
# Streamlit Cloud runs app.py from the repo root, but the working directory is
# not guaranteed to be on sys.path in every environment. Pin it explicitly.
# ---------------------------------------------------------------------------
...
def _preflight() -> list[str]:
    """Return human-readable problems with the deployed file tree."""
    problems: list[str] = []
    for pkg, modules in _REQUIRED_PACKAGES.items():
        pkg_dir = os.path.join(_APP_DIR, pkg)
        if not os.path.isdir(pkg_dir):
            problems.append(f"**`{pkg}/` folder is missing entirely.**")
            continue
        init_path = os.path.join(pkg_dir, "__init__.py")
        if not os.path.exists(init_path):
            problems.append(f"`{pkg}/__init__.py` is missing.")
        elif os.path.getsize(init_path) == 0:
            problems.append(
                f"`{pkg}/__init__.py` is empty (0 bytes) — some upload tools drop these."
            )
        for mod in modules:
            if not os.path.exists(os.path.join(pkg_dir, mod)):
                problems.append(f"`{pkg}/{mod}` is missing.")
    return problems


_problems = _preflight()
if _problems:
    st.set_page_config(page_title="Setup incomplete", page_icon="⚠️", layout="centered")
    st.error("### ⚠️ Deployment is missing files")
    st.markdown(
        "The app cannot start because part of the project did not reach the server. "
        "This is almost always caused by uploading files to GitHub through the web "
    ...
    st.markdown("---")
    st.markdown("#### Fix: push the whole project with git")
    st.code(
        "cd path/to/legal_assistant\n"
        "git init\n"
        "git add -A            # -A includes subfolders\n"
    ...
    st.markdown(
        "Then confirm on GitHub that `legal/`, `llm/` and `ingest/` each appear as "
        "folders containing a non-empty `__init__.py`."
    )

    with st.expander("What the server can actually see right now"):
        try:
            listing = []
            for root, dirs, files in os.walk(_APP_DIR):
                dirs[:] = [d for d in dirs if d not in {"__pycache__", ".git"}]
                depth = root.replace(_APP_DIR, "").count(os.sep)
                if depth > 2:
                    continue
                rel = os.path.relpath(root, _APP_DIR)
                listing.append(f"{rel}/" if rel != "." else ".")
                for f in sorted(files):
                    size = os.path.getsize(os.path.join(root, f))
                    listing.append(f"    {f}  ({size} bytes)")
            st.code("\n".join(listing) or "(empty)", language="text")
        except Exception as exc:  # noqa: BLE001
            st.write(f"Could not list files: {exc}")

    st.caption("© 2026 Dr Shantanu Samanta. All rights reserved.")
    st.stop()
